...

{
  /* TODO */
    ...
    // Histogram: array of #values in each row.
    ...
    // Create csr_row_ptr.
    *csr_row_ptr = (unsigned int*)malloc(1 + sizeof(unsigned int) * m);
    *csr_col_ind = (unsigned int*)malloc(sizeof(unsigned int) * nnz);
    *csr_vals = (double*)malloc(sizeof(double) * nnz);

    int x = 1;
    for(i = 0; i < m; i++){
    	// Populate csr_row_ptr array
	(*csr_row_ptr)[i] = x;
	x += histogram[i];
    }
    (*csr_row_ptr)[i] = x;

    int* row_offsets = (int*)calloc(m, sizeof(int));
    for(i = 0; i < nnz; i++){
	int row = row_ind[i];
    	int col = col_ind[i];
    	double value = val[i];
	int row_start = (*csr_row_ptr)[row - 1] - 1;
	// How many nnz already added from current row
        int row_offset = row_offsets[row - 1];
	(*csr_col_ind)[row_start + row_offset] = col;
	(*csr_vals)[row_start + row_offset] = value;
	row_offsets[row - 1] += 1;
    }
    free(row_offsets);
    free (histogram);
          ...
          double* vector_x, double* res)
{
  ...
  for(int i = 0; i < n; i++) {
    // Initialize res array to 0.
    res[i] = 0;
  }
  
  // Loop through the rows
  for(int i = 0; i < m; i++){
    int row_start_0  = csr_row_ptr[i] - 1;
    int row_end_0 = csr_row_ptr[i+1] - 1;
    // Get start and end index into col and val
    for(int k_0 = row_start_0; k_0 < row_end_0; k_0++){
      int col_0 = csr_col_ind[k_0] - 1;
      double val = csr_vals[k_0];
      double x = vector_x[col_0];
      res[i] += val * x;
    }
  }
}
